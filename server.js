...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    const aiResponse = await callOpenRouter(prompt, { reasoning: false });
...
app.post("/api/variants/:id/export", async (req, res) => {
  try {
    ...
    res.set(
      "Content-Disposition",
      `attachment; filename="variant-${variant.id}.png"`,
    );
    res.send(png);
  } catch (err) {
  ...
  const response = await callOpenRouter(
    {
      messages: [
        {
          role: "system",
          content: `You are a senior design analyst. Given an HTML+CSS ad design, write a detailed description of its visual characteristics in 4-6 sentences. Cover ALL of these aspects:
- Layout structure (centered, split, asymmetric, full-bleed image, grid, columns, stacked, overlapping elements)
- Text treatment (font sizes, weight contrasts, text alignment, text over image vs separate block, uppercase/lowercase, letter spacing, line height, text shadows)
- Color usage (dominant colors, contrast levels, gradients, transparency, color mood)
- Image usage (hero image, background cover, product cutout, text-only, heavy imagery, image cropping, filters/overlays)
- Composition & hierarchy (what dominates visually, CTA size/shape/position, visual flow top-to-bottom or center-out, whitespace distribution)
- Design density (minimal/airy vs packed/busy, padding, margins, breathing room)
- Mood & tone (dark/moody, bright/clean, premium/luxury, playful, bold/aggressive, elegant, corporate)
- What makes this design effective or ineffective (conversion potential, visual appeal, readability)
...
Reply with ONLY the description. No code, no HTML, no prefixes.`,
  ...
  console.log(
    `📝 Style summary updated (${label}): ${description.substring(0, 80)}...`,
  );
}
...
- DO NOT use external resources or CDN links — only local image paths from the list below`;
  ...
  let userPrompt = `Generate 3 unique, high-quality ad designs for the ${dims.label} format.`;
...
async function callOpenRouter(prompt, opts = {}) {
  ...
  const response = await fetch(
    "https://openrouter.ai/api/v1/chat/completions",
    {
      ...
      body: JSON.stringify({
        ...
        ...(opts.reasoning === false ? { reasoning: { effort: "none" } } : {}),
      }),
    },
  );
  ...
  if (data.usage) {
    ...
    console.log(
      `💰 Tokens — in: ${prompt_tokens}, out: ${completion_tokens}, total: ${total_tokens}`,
    );
  }
...
function parseAiResponse(response, format) {
  ...
  try {
    ...
    console.log(
      `⚠ AI response was truncated. Salvaged ${variants.length} of 6 variants.`,
    );
  }
...
async function renderToPng(variant, dims) {
  ...
  let variantHtml = (variant.html || "").replace(
    /\/uploads\//g,
    `http://localhost:${port}/uploads/`,
  );
  let variantCss = (variant.css || "").replace(
    /\/uploads\//g,
    `http://localhost:${port}/uploads/`,
  );
